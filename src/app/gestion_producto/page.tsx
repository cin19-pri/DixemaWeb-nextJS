...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const cargarProductos = () => {
      ...
      const listaSinRepetidos = listaFinal.filter(
        (valorActual: Producto, indice: number, arrayCompleto: Producto[]) => {
          return (
            arrayCompleto.findIndex(
              (t: Producto) => t.nombre === valorActual.nombre,
            ) === indice
          );
        },
      );
    ...
    <div className="dashboard-wrapper" key={pathname}>
      ...
      <aside className={`sidebar ${isMenuOpen ? "open" : ""}`}>
        <div className="logo-section">
          <span className="logo-text">Dixema</span>
        </div>
        <nav className="nav-menu">
          <Link
            href="/gestion_producto"
            className={`nav-item ${pathname === "/gestion_producto" ? "active" : ""}`}
          >
            <Package size={20} />
            <span>Gestión</span>
          </Link>
          ...
          <Link
            href="/nuevo_producto"
            className={`nav-item ${pathname === "/nuevo_producto" ? "active" : ""}`}
          >
            <PlusCircle size={20} />
            <span>Añadir nuevo producto</span>
          </Link>
          <Link
            href="/notificacionesprovedor"
        ...
        <section className="management-section">
          <div className="action-bar-text">
            <button
              ...
              onClick={() => router.push("/nuevo_producto")}
            >
              ...
              <div key={prod.id} className="product-card">
                ...
                <div className="product-info">
                  ...
                  <p className="price-info">
                    ${prod.precio ?? prod.precioMenudeo} MXN /{" "}
                    {prod.unidades ?? prod.stock} unidades
                  </p>
                  <hr />
                  <div className="status-row">
                    ...
                    <div className="action-buttons-group">
                      <span
                        className={`status-badge ${(prod.estatus || "").toLowerCase()}`}
                      >
                        {prod.estatus}
                      </span>
      ...
      <aside className="right-panel">
        ...
        <div className="calendar-widget">
          <div className="calendar-header">
            ...
            <h3>
              {mesNombres[fechaActual.getMonth()]} {fechaActual.getFullYear()}
            </h3>
          </div>
          <div className="calendar-grid">
            {diasSemana.map((dia) => (
              <span key={dia} className="day-name">
                {dia}
              </span>
            ))}
            ...
            {Array.from({ length: totalDiasMes }).map((_, i) => (
              <span
                key={i + 1}
                className={`day-number ${esHoy(i + 1) ? "today" : ""}`}
              >
                {i + 1}
              </span>
            ))}
